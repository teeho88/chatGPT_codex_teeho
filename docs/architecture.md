...
An active turn has separate model-progress and runtime-liveness clocks. Adapter text, reasoning,
tool calls, and tool results reset the progress clock. Owner/helper heartbeats reset only the
liveness clock. A token-owned user wait has an opaque `wait_id`, kind, entry time, and owner turn
identity. While that wait is active, the bridge continues downstream `response.heartbeat` frames
and pauses only the progress-stall watchdog; missing owner liveness still fails with
`runtime_liveness_timeout`. Resuming requires the same owner and `wait_id`, resets the progress
baseline, and stale or duplicate resumes fail closed. Wait controls are internal metadata and never
become assistant content or Responses output items.
