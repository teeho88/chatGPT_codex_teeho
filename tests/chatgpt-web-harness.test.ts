...
test("user wait pauses semantic progress while retaining independently refreshed runtime liveness", () => {
  const progress = new ChatGptExternalTurnProgress();
  const wait = { waitId: "wait_1", kind: "user_input" as const, enteredAt: 1_000, ownerTurnId: "turn_1" };
  progress.beginUserWait(wait);
  expect(progress.snapshot()).toMatchObject({ revision: 1, phase: "waiting_for_user", wait });
  expect(chatGptExternalProgressIsLive(progress.snapshot(), 99_000, 1)).toBeTrue();
  progress.recordHeartbeat(99_000);
  expect(progress.snapshot().lastLivenessAt).toBe(99_000);
  progress.resumeUserWait(wait.waitId, 100_000);
  expect(progress.snapshot()).toMatchObject({ revision: 2, lastProgressAt: 100_000 });
  expect(progress.snapshot().phase).toBeUndefined();
  expect(() => progress.resumeUserWait(wait.waitId, 100_001)).toThrow("does not match");
});
