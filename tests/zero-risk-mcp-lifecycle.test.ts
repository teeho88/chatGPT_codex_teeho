...
describe("Zero Risk turn broker lifecycle", () => {
  ...
  test("exposes the same lifecycle through RemoteTurnBroker", async () => {
    ...
    try {
      ...
      const wait = { waitId: "remote_wait", kind: "manual_step" as const, enteredAt: 1_000, ownerTurnId: requestId };
      await remote.beginUserWait(requestId, wait);
      expect(await remote.snapshotUserWait(requestId)).toEqual(wait);
      await expect(remote.resumeUserWait(requestId, "stale_wait")).rejects.toThrow("does not match");
      await remote.resumeUserWait(requestId, wait.waitId);
      expect(await remote.snapshotUserWait(requestId)).toBeUndefined();
