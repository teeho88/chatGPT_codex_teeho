...
test("turn broker user waits are owner-bound and reject stale resumes", async () => {
  ...
  try {
    ...
    const wait = { waitId: "wait_1", kind: "approval" as const, enteredAt: 1_000, ownerTurnId: token };
    broker.beginUserWait(token, wait);
    expect(broker.snapshotUserWait(token)).toEqual(wait);
    broker.beginUserWait(token, wait);
    expect(() => broker.resumeUserWait(token, "wait_old")).toThrow("does not match");
    broker.resumeUserWait(token, wait.waitId);
    expect(broker.snapshotUserWait(token)).toBeUndefined();
    expect(() => broker.resumeUserWait(token, wait.waitId)).toThrow("does not match");
  } finally {
    await broker.close();
    rmSync(root, { recursive: true, force: true });
  }
