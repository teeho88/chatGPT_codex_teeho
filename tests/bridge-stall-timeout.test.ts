...
function bridged(
  ...
  return bridgeToResponsesSSE(
    ...
    undefined,
    heartbeatMs,
    { streamPlatform: "darwin", stallTimeoutSec, ...(now ? { now } : {}), ...(livenessTimeoutSec ? { livenessTimeoutSec } : {}) },
  );
}
...
test("heartbeats preserve runtime liveness but do not mask a running progress stall", async () => {
...
test("a waiting user turn outlives the progress budget while heartbeats continue", async () => {
  async function* waiting(): AsyncGenerator<AdapterEvent> {
    yield { type: "user_wait_started", wait: { waitId: "wait_1", kind: "user_input", enteredAt: 0, ownerTurnId: "turn_1" } };
    for (let beat = 0; beat < 20; beat++) {
      await sleep(20);
      yield { type: "heartbeat" };
    }
    yield { type: "user_wait_ended", waitId: "wait_1", outcome: "resumed", endedAt: 1_000 };
    yield { type: "text_delta", text: "answer" };
    yield { type: "done", endTurn: true };
  }

  const body = await new Response(bridged(waiting(), 0.05, 10, undefined, 1)).text();
...
test("a waiting user turn fails when its runtime stops heartbeating", async () => {
  async function* abandoned(): AsyncGenerator<AdapterEvent> {
    yield { type: "user_wait_started", wait: { waitId: "wait_1", kind: "approval", enteredAt: 0, ownerTurnId: "turn_1" } };
    await sleep(250);
  }

  const body = await new Response(bridged(abandoned(), 0.05, 10, undefined, 0.1)).text();
  expect(body).toContain("runtime_liveness_timeout");
});
