...
test("a waiting user phase suppresses DOM health only while its liveness heartbeat is fresh", () => {
  const wait = { waitId: "wait_1", kind: "approval" as const, enteredAt: 1_000, ownerTurnId: "turn_1" };
  const snapshot = {
    revision: 2, lastToolBatchRevision: 0, activeToolCalls: 0,
    phase: "waiting_for_user" as const, wait, lastLivenessAt: 100_000,
  };
  expect(chatGptExternalProgressSuppressesDomHealth(snapshot, 100_001)).toBeTrue();
  expect(chatGptExternalProgressSuppressesDomHealth(
    snapshot, 100_000 + CHATGPT_EXTERNAL_PROGRESS_STALL_CEILING_MS,
  )).toBeFalse();
});
