...
interface BrokerRequest {
  ...
  method:
    ...
    | "owner_revoke"
    | "owner_user_wait_begin"
    | "owner_user_wait_resume"
    | "owner_user_wait_cancel"
    | "owner_user_wait_snapshot"
...
export interface TurnBrokerOwner {
  ...
  beginUserWait(token: string, wait: TurnUserWait): void | Promise<void>;
  resumeUserWait(token: string, waitId: string): void | Promise<void>;
  cancelUserWait(token: string, waitId: string, reason?: Error): void | Promise<void>;
  snapshotUserWait(token: string): TurnUserWait | undefined | Promise<TurnUserWait | undefined>;
...
export class TurnBroker implements TurnBrokerOwner {
  ...
  beginUserWait(token: string, wait: TurnUserWait): void {
    this.prune();
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    if (wait.ownerTurnId !== token || !wait.waitId || !Number.isFinite(wait.enteredAt)) {
      throw new Error("ChatGPT user wait does not belong to this turn");
    }
    if (channel.userWait?.waitId === wait.waitId) return;
    if (channel.userWait) throw new Error("ChatGPT turn already has an active user wait");
    channel.userWait = structuredClone(wait);
  }

  resumeUserWait(token: string, waitId: string): void {
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    if (!channel.userWait || channel.userWait.waitId !== waitId) {
      throw new Error("ChatGPT user wait resume does not match the active wait");
    }
    delete channel.userWait;
  }

  cancelUserWait(token: string, waitId: string, reason = new Error("ChatGPT user wait was cancelled")): void {
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    if (!channel.userWait || channel.userWait.waitId !== waitId) {
      throw new Error("ChatGPT user wait cancellation does not match the active wait");
    }
    delete channel.userWait;
    this.rejectChannel(channel, reason);
  }

  snapshotUserWait(token: string): TurnUserWait | undefined {
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    return channel.userWait && structuredClone(channel.userWait);
  }
  ...
  private async dispatch(request: BrokerRequest, socketSignal?: AbortSignal): Promise<unknown> {
    ...
    if (request.method === "owner_user_wait_begin") {
      if (!request.token || !request.wait) throw new Error("turn owner wait is required");
      this.beginUserWait(request.token, request.wait);
      return { started: true };
    }
    if (request.method === "owner_user_wait_resume") {
      if (!request.token || !request.waitId) throw new Error("turn owner wait id is required");
      this.resumeUserWait(request.token, request.waitId);
      return { resumed: true };
    }
    if (request.method === "owner_user_wait_cancel") {
      if (!request.token || !request.waitId) throw new Error("turn owner wait id is required");
      this.cancelUserWait(request.token, request.waitId);
      return { cancelled: true };
    }
    if (request.method === "owner_user_wait_snapshot") {
      if (!request.token) throw new Error("turn owner token is required");
      return { wait: this.snapshotUserWait(request.token) ?? null };
    }
...
export class RemoteTurnBroker implements TurnBrokerOwner {
  ...
  async beginUserWait(token: string, wait: TurnUserWait): Promise<void> {
    const response = await callTurnBroker<{ started?: unknown }>(this.socketPath, { method: "owner_user_wait_begin", token, wait });
    if (response.started !== true) throw new Error("DEV turn owner received an invalid user-wait start result");
  }

  async resumeUserWait(token: string, waitId: string): Promise<void> {
    const response = await callTurnBroker<{ resumed?: unknown }>(this.socketPath, { method: "owner_user_wait_resume", token, waitId });
    if (response.resumed !== true) throw new Error("DEV turn owner received an invalid user-wait resume result");
  }

  async cancelUserWait(token: string, waitId: string): Promise<void> {
    const response = await callTurnBroker<{ cancelled?: unknown }>(this.socketPath, { method: "owner_user_wait_cancel", token, waitId });
    if (response.cancelled !== true) throw new Error("DEV turn owner received an invalid user-wait cancellation result");
  }

  async snapshotUserWait(token: string): Promise<TurnUserWait | undefined> {
    const response = await callTurnBroker<{ wait?: unknown }>(this.socketPath, { method: "owner_user_wait_snapshot", token });
    if (response.wait === null) return undefined;
    const wait = response.wait as Partial<TurnUserWait> | undefined;
    if (!wait || typeof wait.waitId !== "string" || typeof wait.ownerTurnId !== "string" || !Number.isFinite(wait.enteredAt)
      || !["approval", "user_input", "manual_step"].includes(wait.kind ?? "")) {
      throw new Error("DEV turn owner received an invalid user-wait snapshot");
    }
    return wait as TurnUserWait;
  }
