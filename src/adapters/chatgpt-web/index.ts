...
function emitTraceEvents(trace: ChatGptTraceEvent[], emit: (event: AdapterEvent) => void): void {
  for (const event of trace) {
    if (event.kind === "user_wait_started") {
      emit({ type: "user_wait_started", wait: event.wait });
      continue;
    }
    if (event.kind === "user_wait_ended") {
      emit({ type: "user_wait_ended", waitId: event.waitId, outcome: event.outcome, endedAt: event.endedAt });
      continue;
    }
      ...
      const runManual = async (): Promise<string> => {
        try {
          ...
          const waitHeartbeat = setInterval(() => {
            externalProgress.recordHeartbeat();
            trace.push({ kind: "heartbeat" });
          }, 2_000);
          try {
          ...
          await session.runExclusive(async () => {
            ...
            if (settled) {
              ...
              if (finalReplay.length > 0) {
                ...
                session.appendRoundReasoning(roundKey, trace.flatMap(event => "text" in event ? [event.text] : []));
            ...
            try {
              ...
              const emitNewTrace = (trace: ChatGptTraceEvent[]) => {
                const traceText = trace.flatMap(event => "text" in event ? [event.text] : []);
                roundReasoning.push(...traceText);
                session.appendRoundReasoning(roundKey, traceText);
                emitRoundBatch(buffer => emitTraceEvents(trace, buffer));
              };
