export interface ChatGptExternalTurnProgressSnapshot {
  ...
  /** Missing in legacy helper frames and therefore interpreted as `running`. */
  phase?: "running" | "waiting_for_user";
  wait?: import("../../types").TurnUserWait;
...
export class ChatGptExternalTurnProgress extends ChatGptTurnProgressBroadcaster {
  ...
  private lastLivenessAt?: number;
  private wait?: import("../../types").TurnUserWait;
  ...
  snapshot(): ChatGptExternalTurnProgressSnapshot {
    return {
      ...
      ...(this.wait ? { phase: "waiting_for_user" as const, wait: { ...this.wait } } : {}),
      ...(this.lastLivenessAt !== undefined ? { lastLivenessAt: this.lastLivenessAt } : {}),
    };
  }
  ...
  recordHeartbeat(now = Date.now()): void {
    this.assertNotRetired();
    if (!Number.isFinite(now)) throw new Error("ChatGPT external liveness timestamp must be finite");
    this.lastLivenessAt = now;
    // This is a transport revision, not semantic model progress: mirrors need a new frame to
    // observe liveness, while lastProgressAt remains untouched.
    this.revision += 1;
    this.notify(this.snapshot());
  }

  beginUserWait(wait: import("../../types").TurnUserWait): void {
    this.assertNotRetired();
    if (this.wait && this.wait.waitId !== wait.waitId) throw new Error("ChatGPT external progress already has a user wait");
    if (this.wait) return;
    this.wait = { ...wait };
    this.lastLivenessAt = wait.enteredAt;
    this.revision += 1;
    this.notify(this.snapshot());
  }

  resumeUserWait(waitId: string, now = Date.now()): void {
    this.assertNotRetired();
    if (!this.wait || this.wait.waitId !== waitId) throw new Error("ChatGPT user wait resume does not match the active wait");
...
export class ChatGptMirroredTurnProgress extends ChatGptTurnProgressBroadcaster {
  ...
  apply(next: ChatGptExternalTurnProgressSnapshot): boolean {
    ...
    if (next.lastToolBatchRevision < this.current.lastToolBatchRevision
      ...
      || (this.current.phase === "waiting_for_user" && (next.phase ?? "running") === "running"
        && next.lastProgressAt === this.current.lastProgressAt)) {
...
export function assertChatGptTurnProgressSnapshot(
  ...
  if (!value
    ...
    || (value.phase !== undefined && value.phase !== "running" && value.phase !== "waiting_for_user")
    || ((value.phase === "waiting_for_user") !== Boolean(value.wait))
    || (value.wait !== undefined && (!value.wait.waitId || !value.wait.ownerTurnId || !Number.isFinite(value.wait.enteredAt)))
    || (value.lastProgressAt !== undefined && !Number.isFinite(value.lastProgressAt))
    || (value.lastLivenessAt !== undefined && !Number.isFinite(value.lastLivenessAt))
...
export function chatGptExternalRuntimeIsLive(
  ...
  if (!snapshot) return false;
  if (!Number.isFinite(now) || !Number.isFinite(graceMs) || graceMs < 0) {
    throw new Error("ChatGPT runtime liveness inputs are invalid");
  }
  const lastLivenessAt = snapshot.lastLivenessAt ?? snapshot.lastProgressAt;
  return lastLivenessAt !== undefined && now - lastLivenessAt < graceMs;
}
