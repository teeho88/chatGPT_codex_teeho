...
import type { AdapterEvent, CodexParsedRequest, TurnUserWait } from "../../types";
...
export type ChatGptTraceEvent = {
  kind: "reasoning" | "commentary";
  text: string;
  continuation?: boolean;
} | { kind: "user_wait_started"; wait: TurnUserWait }
  | { kind: "user_wait_ended"; waitId: string; outcome: "resumed" | "denied" | "cancelled"; endedAt: number }
  | { kind: "heartbeat" };
...
export class ChatGptTraceFeed {
  ...
  push(event: ChatGptTraceEvent): void {
    if (event.kind === "user_wait_started" || event.kind === "user_wait_ended" || event.kind === "heartbeat") {
