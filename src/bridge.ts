import type { AdapterEvent, CodexMessagePhase, CodexProviderContinuationState, CodexUsage, TurnUserWait } from "./types";
...
export function bridgeToResponsesSSE(
  ...
  options?: {
    ...
    /** Runtime liveness budget. Heartbeats reset this without counting as model progress. */
  ...
  const emit = (name: string, data: Record<string, unknown>) => {
      ...
      const step = async () => {
        ...
        while (!terminated && !closed && emittedFrames === emittedAtStart) {
          ...
          adapterEventCount += 1;
          if (event.type !== "heartbeat" && event.type !== "user_wait_started" && event.type !== "user_wait_ended") {
            progressBaselineAt = lastLivenessAt;
            stallWarned = false;
          }
          if (event.type === "user_wait_started") {
            if (waitState && waitState.waitId !== event.wait.waitId) {
              throw new Error("user_wait_started conflicts with the active wait");
            }
            waitState = event.wait;
            continue;
          }
          if (event.type === "user_wait_ended") {
            if (!waitState || waitState.waitId !== event.waitId) {
              throw new Error("user_wait_ended does not match the active wait");
            }
            waitState = undefined;
            progressBaselineAt = event.endedAt;
            stallWarned = false;
            if (event.outcome !== "resumed") {
              throw new Error(`user wait ended with ${event.outcome}`);
            }
            continue;
          }
      ...
      const startStream = () => {
        ...
        beat = setInterval(() => {
          ...
          const checkedAt = now();
          const livenessSilenceMs = checkedAt - lastLivenessAt;
          if (livenessSilenceMs >= livenessTimeoutMs) {
            console.error(
              `[bridge] runtime_liveness_timeout model=${modelId} response=${responseId}`
              + ` phase=${waitState ? "waiting_for_user" : "running"} sinceLastLivenessMs=${livenessSilenceMs}`,
            );
            emit("response.failed", {
              response: {
                ...responseSnapshot("failed", finishedItems),
                error: responseError(504, "upstream_error", "ChatGPT runtime stopped sending liveness heartbeats."),
                last_error: responseError(504, "upstream_error", "ChatGPT runtime stopped sending liveness heartbeats."),
                incomplete_details: { reason: "runtime_liveness_timeout" },
              },
            });
            reportTerminal("failed");
            onCancel?.();
          ...
          if (waitState) {
            try { controller.enqueue(heartbeatFrame); emittedFrames++; } catch { closed = true; }
            return;
          }
          const silenceMs = checkedAt - progressBaselineAt;
          if (silenceMs >= stallTimeoutMs / 2 && !stallWarned) {
            ...
            console.warn(
              ...
              + ` lastEvent=${lastAdapterEventType} sinceLastProgressMs=${silenceMs}`,
            );
          }
