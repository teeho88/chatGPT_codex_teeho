...
/** A turn-owned pause that can only be resolved by the same user interaction. */
export interface TurnUserWait {
  waitId: string;
...
export type AdapterEvent =
  | { type: "heartbeat" }
  /** Internal lifecycle controls. They never become public Responses output items. */
  | { type: "user_wait_started"; wait: TurnUserWait }
  | { type: "user_wait_ended"; waitId: string; outcome: "resumed" | "denied" | "cancelled"; endedAt: number }
...
export interface CodexProviderConfig {
  ...
  chatgptWeb?: {
    ...
    /** Private directory containing durable checkpoints for explicit user waits. */
